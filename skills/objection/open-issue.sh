...
# Empty, or one line that says nothing is open ("nothing.", "Nothing open.",
# "(none)", "n/a"): nothing to track. More lines, or anything else, is.
if [ -z "$open" ] || { [ "$(printf '%s\n' "$open" | wc -l | tr -d ' ')" = 1 ] &&
  printf '%s\n' "$open" | grep -qiE '^[[:space:]]*[(]?(nothing|none|n/a)([^[:alnum:]]|$)'; }; then
...
${branch:+Branch \`$branch\`, }commit \`${sha:0:7}\`${pr_url:+, PR $pr_url}.
...
# Search finds an old one by the short SHA in its title (it does not index
# HTML comments) but lags behind a new one, which the latest issues cover.
# A failed lookup stops here: creating anyway could open a duplicate.
found=$( { "$gh_bin" issue list --state all --search "${sha:0:7} in:title" --json url,body &&
  "$gh_bin" issue list --state all --limit 100 --json url,body; } ) ||
...
existing=$(printf '%s' "$found" | MARK="$marker" node -e '
  let s = ""; process.stdin.on("data", (d) => (s += d)).on("end", () => {
    // Two JSON arrays, one per lookup, one after the other.
    let lists;
    try { lists = JSON.parse("[" + s.trim().replace(/\]\s*\[/g, "],[") + "]"); } catch { process.exit(3); }
    const i = lists.flat().find((x) => (x.body || "").includes(process.env.MARK));
    if (i) console.log(i.url);
